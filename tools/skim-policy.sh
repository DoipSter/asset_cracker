#!/usr/bin/env bash
# Show or set the skim policy: what share of a bucket's gain above its high-water mark goes to
# each money bucket. Percentages; what is left stays in the bucket and compounds.
#
#   tools/skim-policy.sh                                  # show the policy in force and the pools (prod)
#   tools/skim-policy.sh set 20 10 25 0 "first guess"     # winnings replenishment tax fees, then a note
...
if [ "${1:-}" = "set" ]; then
  [ $# -ge 5 ] || { echo "usage: $0 set <winnings%> <replenishment%> <tax%> <fees%> [note]"; exit 2; }
...
select effective_at::timestamp(0) as since, (winnings_bps / 100.0)::numeric(5,2) as "winnings %", (replenish_bps / 100.0)::numeric(5,2) as "replenishment %",
       (tax_bps / 100.0)::numeric(5,2) as "tax %", (fees_bps / 100.0)::numeric(5,2) as "fees %",
